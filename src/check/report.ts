import type { CheckScenario, StepResult, MockResult } from './scenarios';
...
export interface CheckCase {
  ...
  scenario?: string; steps?: StepResult[]; mocks?: MockResult[];
}
...
export const caseId = (path: string, viewport: CheckViewport, auth: boolean, scenario?: CheckScenario) => hash([path, viewport.name, viewport.width, viewport.height, viewport.colorScheme ?? 'light', auth, ...(scenario ? [canonical(scenario)] : [])]);
...
export function renderCheckReport(report: CheckReport): string {
  ...
  const cases = report.cases.map(c => `<tr><td>${escape(c.name)}</td><td>${escape(c.viewport.name)} (${c.viewport.width} × ${c.viewport.height})</td><td>${escape(c.status)}</td><td>${escape(c.message ?? `${c.report?.findings.length ?? 0} findings`)}${c.steps ? `<details><summary>Scenario steps</summary><ol>${c.steps.map(step => `<li>${escape(step.status)} · ${escape(step.action)} <code>${escape(step.selector)}</code></li>`).join('')}</ol>${c.mocks?.length ? `<ul>${c.mocks.map(mock => `<li>${escape(mock.method)} ${escape(mock.path)}: ${mock.calls} request(s), ${mock.expectedResponses} response(s) required</li>`).join('')}</ul>` : ''}</details>` : ''}</td></tr>`).join('');
